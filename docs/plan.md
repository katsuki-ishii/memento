...
- [ ] フロントエンド実装 (MVP)
  ...
  - [ ] 認証（Hosted UI 前提）
    - [ ] Hosted UI リダイレクト導線をログイン/サインアップ共通で設計・配置（自前フォーム不要）
    - [ ] 認可コード受領と `/oauth2/token` でのトークン交換処理を実装（既存 stub を仕上げ）
    - [ ] ローカルセッション保持とリダイレクト後遷移（初期設定 or ダッシュボード）
    - [ ] ログアウト: Cognito `/logout` リダイレクト + ローカルセッション/ストアクリア + トースト通知
    - [ ] UI文言/ボタン: 最小限の説明とリンク整備（サインアップ・ログイン共通ボタン）
  - [ ] UI修正
    - [x] サイドメニューが表示されない
