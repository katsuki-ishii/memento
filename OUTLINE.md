...
- **認証**: Cognito Hosted UI + PKCE。成功後は初期設定へ（設定済みならダッシュボードへ直行）。
...
3. **初期設定を入力**: username・生年・寿命・週開始・テーマを保存。完了でダッシュボードへ。
4. **ダッシュボードを見る**: 寿命×52週グリッドで現在週を確認し、過去（使った時間）と未来（残りの時間）を色分け表示。
