...
- ルートディレクトリのdocsを毎回確認し、実装と各種ルールや要件、docs/mvp/plan.mdの進捗に不整合が起きないようにする。
...
- **デプロイコマンド**: `cd backend && sam deploy --stack-name memento-dev --resolve-s3 --capabilities CAPABILITY_IAM --no-confirm-changeset --no-fail-on-empty-changeset --parameter-overrides "CallbackUrls=https://d1ney9ei4kzmv9.cloudfront.net/auth/callback,http://localhost:5173/auth/callback" "LogoutUrls=https://d1ney9ei4kzmv9.cloudfront.net/auth/start,http://localhost:5173/auth/start" CorsAllowOrigin=https://d1ney9ei4kzmv9.cloudfront.net`
- **注意**: CognitoのコールバックURLとログアウトURLは開発環境（localhost）と本番環境（CloudFront）の両方を登録する
